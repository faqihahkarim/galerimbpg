...
if ($action === 'edit') {
    ...
    try {
        ...
        // 1. Handle slot change if requested
        ...
        // 4. Handle activity updates if provided
        ...
        if ($currentBooking['organization_name'] != $organization_name) {
            $changes[] = "organisasi";
        }
        if ($currentBooking['contact_person'] != $contact_person) {
            $changes[] = "pegawai";
        }
        if ($currentBooking['phone_number'] != $phone_number) {
            $changes[] = "telefon";
        }
        if ($currentBooking['email'] != $email) {
            $changes[] = "emel";
        }
        if ($currentBooking['total_participants'] != $total_participants) {
            $changes[] = "peserta";
        }
        if ($old_package_id != $new_package_id) {
            $changes[] = "pakej";
        }
        if ($new_slot_id > 0 && $old_slot_id != $new_slot_id) {
            $changes[] = "slot";
        }
        if (isset($_POST['activity_participants'])) {
            $changes[] = "aktiviti";
        }
